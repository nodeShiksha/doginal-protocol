...
async function showViewDoginalPage(inscriptionId) {
  ...
  $("#doginal_send_button").onclick = async () => {
    ...
    const inscription = (await chrome.storage.local.get(key))[key];
  ...
  $("#doginal_inscription_number").onclick = () => window.open(`${__URL__}shibescription/${inscriptionId}`);
...
const realodButton = $('.reload-page');

realodButton.onclick = function() {
  location.reload();
}